...
%%% helper functions %%%
%%%%%%%%%%%%%%%%%%%%%%%%
function pruned = pruneMap(pruned, map)
    [center,boundary] = getBoxes(map.blocks); % get obstacle constraints to pass along to functions
    %%% iterative pruning
    ...
    for j=1:6
        ...
        pruned = pruneForward(refill(end:-1:1,:), map, center, boundary);
        ...
        pruned = pruneBackward(refill, map, center, boundary);
    end
    
    %%% final point refill for close path fit with polynomials in trajectory_generator.m
    refill = [pruned(1,:)];
    for i=1:numel(pruned(:,1))-1
        current = pruned(i,:);
        next = pruned(i+1,:);
...
%Forwards
function pruned = pruneForward(path, map, center, boundary)
    % from the start step forwards until right before obstructed path
    pathIdx = 1;
    current = path(pathIdx, :);
    pruned = [current];
    % continue until at the end of path
    while ~all(current == path(end, :))
        nextIdx = pathIdx+1;
        next = path(nextIdx, :);
        % cycle through points along path until collision 
        while (nextIdx + 1 <= numel(path(:,1))) && (nextIdx == pathIdx + 1 || ~checkCollision(map, generatePoints(current, next, map.res_xyz/10), center, boundary))
            nextIdx = nextIdx + 1;
            next = path(nextIdx, :);
        end
...
%Backwards
function pruned = pruneBackward(path, map, center, boundary)
    % from then end step backwards until there is an unobstructed path to a point
    pathIdx = 1;
    current = path(pathIdx, :);
    pruned = [current];
    % continue until at the end of path
    while any(current ~= path(end, :)) 
        nextIdx = numel(path(:,1));
        next = path(nextIdx, :);
        % cycle through points along path until collision 
        while (nextIdx - 1 ~= pathIdx) && checkCollision(map, generatePoints(current, next, map.res_xyz/10), center, boundary)
            nextIdx = nextIdx - 1;
            next = path(nextIdx, :);
        end
        % append path
...
function collides = checkCollision(map, points, c, b)
    % geometrically checking to see if points on a line intersect with obstacles
    collides = false;
...
%     end
    for i = 1:numel(points(:, 1))
        if any(all(points(i,:) <= c+b+map.margin & points(i,:) >= c-b-map.margin,2))
           collides = true;
           return
        end
    end
...
function points = generatePoints(p1, p2, res)
    % generate some number of points between waypoints
...
function [c,b] = getBoxes(box)
    % c : center of boxes
    % b : length from center to boundary of box
    c = (box(:,1:3)+box(:,4:6))/2;
    b = c-box(:,1:3);
end
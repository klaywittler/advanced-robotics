classdef QuadrotorROS
    ...
    methods
        ...
        function [quadrotor] = navigate(quadrotor, map, goal)
            clear trajectory_generator
            student_control_nav(quadrotor.data.t, quadrotor.data.qd, map, goal);
            quadrotor.transition('std_trackers/NullTracker');
            quadrotor.data.state = 'navigate';
        end
        
        function [quadrotor] = drynavigate(quadrotor, map, goal)
            clear trajectory_generator
            student_control_nav(quadrotor.data.t, quadrotor.data.qd, map, goal);
            quadrotor.transition('std_trackers/NullTracker');
            quadrotor.data.state = 'drynavigate';
        end
        
        function [quadrotor] = show_path(quadrotor, map, goal)
            start = reshape(quadrotor.data.qd{1}.pos, 1,3);
        ...
        function [quadrotor] = viconCallback(quadrotor, sub, msg)
            ...
            switch quadrotor.data.state
              ...
              case {'navigate', 'drynavigate'}
                desired_state = student_control_nav(t);
            end
            ...
            if ~strcmp(quadrotor.data.state, 'dryrun') && ~strcmp(quadrotor.data.state, 'drynavigate')
        ...
        function plot_maze(quadrotor, map)
            if ~isempty(quadrotor.data.history)
                plot_path(map);
                plot_3d(quadrotor.data.history, []);
            else
